<template>
    ...
    <div class="card rounded shadow-md w-full h-auto py-2" v-show="!show_modal">
        <div class="flex items-start">
            ...
            <text-area :rows='1' class="flex-grow ml-1 mr-4" child_class="w-full" @click="show_modal = true"></text-area>
            <button class="btn btn-viridiant" @click="show_modal = true">Post</button>
        </div>
    ...
    <modal-component v-show="show_modal" size="md">
        <template v-slot:header>
            <div class="border-b pb-3 flex justify-between items-center">
                <h1 class="font-semibold text-onyx-500 text-xl">Create a new post</h1>
                <svg class="feather feather-x cursor-pointer" @click="show_modal = false" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </div>
        </template>
            ...
            <div class="mt-6">
                <resize-auto>
                    <template v-slot:default="{resize}">
                        <textarea
                            class="appearance-none font-light border-none w-full text-onyx-700 bg-cultured-100 resize-none text-xl focus:outline-none max-h-24"
                            @input="resize"
                ...
                <input id="pictures_input" type="file" name="pictures" hidden>
            </div>
        </template>
        <template v-slot:footer>
            <div class="w-full flex justify-between items-center">
                ...
                <button class="btn btn-viridiant-outline">Publish</button>
            </div>
        </template>
    </modal-component>
    </div>
